...
function createTextTexture(gl: WebGLRenderingContext, text: string, font: string, color: string) {
  ...
  const texture = new Texture(gl as unknown as never, { generateMipmaps: false });
...
class Title {
  ...
  constructor(o: any) {
    this.gl = o.gl; this.plane = o.plane; this.renderer = o.renderer;
    this.text = o.text; this.textColor = o.textColor ?? "#3a3a3a"; this.font = o.font ?? "30px sans-serif";
    this.createMesh();
  }
  createMesh() {
...
class Media {
  extra = 0; geometry: any; gl: any; image: string; index: number; length: number; renderer: any; scene: any; screen: any; text: string; viewport: any; bend: number; textColor: string; borderRadius: number; font: string;
  program: any; plane: any; title: any; speed = 0; isBefore = false; isAfter = false;
  scale = 1; padding = 2; width = 0; widthTotal = 0; x = 0;
  constructor(o: any) {
    this.geometry = o.geometry; this.gl = o.gl; this.image = o.image; this.index = o.index; this.length = o.length;
    this.renderer = o.renderer; this.scene = o.scene; this.screen = o.screen; this.text = o.text; this.viewport = o.viewport;
    this.bend = o.bend; this.textColor = o.textColor; this.borderRadius = o.borderRadius ?? 0; this.font = o.font;
  ...
  createShader() {
    ...
    img.src = this.image;
    img.onload = () => {
      texture.image = img as unknown as never;
...
class App {
  ...
  onCheck() { if (!this.medias?.[0]) return; const width = this.medias[0].width; const itemIndex = Math.round(Math.abs(this.scroll.target) / width); const item = width * itemIndex; this.scroll.target = this.scroll.target < 0 ? -item : item; }
  onResize() {
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Recommended upcycling projects gallery"
      className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing focus-visible:outline-2 focus-visible:outline-brand-ink"
    />
  );
}